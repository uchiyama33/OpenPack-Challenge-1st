...
class OpenPackAll(torch.utils.data.IterableDataset):  # FIXME torch.utils.data.ChainDatasetの検討
    ...
    def __iter__(self):
        ...
        for ind in index_order:
            if self.mode == "train":
                if self.cfg.time_data_aug.use:
                    assert self.cfg.model.use_only_one_image
                    max_value = self.cfg.time_data_aug.max_time_step_width
                    # min_value = self.cfg.time_data_aug.min_time_step_width
                    min_value = (
                        self.time_step_width
                    )  # self.time_step_width ~ self.cfg.time_data_aug.max_time_step_widthの間
                    assert max_value >= min_value
                    current_time_step_width = int((max_value - min_value) * torch.rand(1)[0] + min_value)
                else:
                    current_time_step_width = self.time_step_width
            else:
                current_time_step_width = self.time_step_width
            ...
            start_ts = (
                epoch_start_time[data_ind] * current_time_step_width
                + clip_position * self.clip_time * current_time_step_width
            )
            end_ts = (
                epoch_start_time[data_ind] * current_time_step_width
                + (clip_position + 1) * self.clip_time * current_time_step_width
            )
            ...
            imu, _ = self.segment_and_padding(
                ...
                start_ts,
                end_ts,
                self.cfg.dataset.stream.frame_rate_imu,
                current_time_step_width,
            )
            ...
            # label
            label, ts = self.segment_and_padding(
                data["label"],
                data["label_unixtime"],
                start_ts,
                end_ts,
            ...
            if self.cfg.model.use_cnn_feature:
                kinect_depth, exist_data_kinect_depth = self.segment_and_padding_for_feature(
                    ...
                    current_time_step_width,
                    label_ts=ts,
                    # FIXME use_only_one_imageのときにtsを参照しているから、tsをcurrent_time_step_widthに合わせて修正する
                )
            else:
            ...
            if self.cfg.pre_data_aug.use and self.mode == "train":
                imu, keypoint, kinect_depth, rs02_depth = self.pre_data_aug(
                    [imu, keypoint, kinect_depth, rs02_depth],
                    data_names=["imu", "keypoint", "kinect_depth", "rs02_depth"],
                )

            if self.mode == "train":  # dummy
                ts = np.zeros((self.clip_time, 1))
            else:  # padding ts, use only submission
                ts = np.concatenate(
                    [
                        ts,
                        np.arange(
    ...
    def segment_and_padding(
        ...
        if c != None:
            ...
            data = self._segment_and_padding(data, unixtime, fs, current_time_step_width)
            exist_data = self._segment_and_padding(exist_data, unixtime, fs, current_time_step_width)
    ...
    def _segment_and_padding(self, data, ts, fs, current_time_step_width):
        data_segment = []
        num_use_data = int(current_time_step_width / 1000 * fs)
        num_data = int(self.time_step_width / 1000 * fs)
        if num_data == 0:  # for label's unixtime
            num_data = 1
        for i in range(self.clip_time):
            _segment_start = np.where(ts >= ts[0] + (i * current_time_step_width))
            if len(_segment_start[0]) == 0:
                segment_start = ts.shape[0] - 1
            else:
                segment_start = _segment_start[0][0]
            segment_end = segment_start + num_use_data
            ...
            start_end_ind = np.linspace(segment_start, segment_end, num_data).astype(int)
            data_segment.append(data[start_end_ind])
            ...
            if torch.rand(1)[0] < self.cfg.pre_data_aug.flip_p:
                for i, name in enumerate(data_names):
                    if name == "imu":
                        ...
                        x_list[i] = x_list[i].reshape(p, f, 4, 3)[:, :, [1, 0, 3, 2]].reshape(p, f, d)
                    if name == "keypoint":
                        x_list[i][:, :, 0, :] *= -1  # xyの順番合ってるか怪しい
                    elif name in ["kinect_depth", "rs02_depth"]:
                        if x_list[i].ndim == 2:  # rs02 dummy
                            continue
                        p, f, c, h, w = x_list[i].shape
