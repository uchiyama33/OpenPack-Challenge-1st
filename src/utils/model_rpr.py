...
class myRPRTransformer(OpenPackBase):
    def __init__(
        self,
        num_patches,
        num_classes,
        ...
        **kargs,
        ...
        encoder_layer = TransformerEncoderLayerRPR(dim, heads, mlp_dim, dropout, er_len=num_patches)
        encoder = TransformerEncoderRPR(encoder_layer, depth, encoder_norm)
        self.transformer = nn.Transformer(
            d_model=dim, nhead=heads, num_encoder_layers=depth,
            num_decoder_layers=0, dropout=dropout, # activation=self.ff_activ,
            dim_feedforward=mlp_dim, custom_decoder=self.dummy, custom_encoder=encoder
        )
    ...
    def forward(
        ...
        x = self.dropout(x)
        x = self.transformer(src=x, tgt=x, src_mask=mask)
        x = self.to_latent(x)
